...
v0.2.1 可从 GitHub Releases 下载。当前发布包使用 ad-hoc 签名，首次打开前请阅读安装与信任说明：
...
1. 下载 `Vibelsland-Free-0.2.1-macos.zip`。
...
说明：当前免费发布包使用 ad-hoc codesign，首次打开需要按 Gatekeeper 说明手动确认。Developer ID 签名和 notarization 可以作为后续降低首次打开摩擦的分发改进，但当前 v0.2.1 下载、校验、源码和安装说明已经公开。
...
Vibelsland Free v0.2.1 已经具备可下载的本机版本，包含浮岛展示、设置页、Hook 安装、审批窗口、运行状态检查、单实例保护、重启恢复和发布打包脚本。当前免费发布包使用 ad-hoc 签名，并提供公开下载、SHA-256 校验、源码、安装说明和支持入口。
