...
package final class ConversationTranscriptReader: @unchecked Sendable {
    ...
    package func loadSnapshot(from url: URL, source: AgentSource, limit: Int = 8) -> AgentTranscriptSnapshot? {
        guard let tail = try? readTailData(from: url, maxBytes: tailReadBytes) else {
            return nil
        }
        ...
        for data in tailLines(from: tail.data, startsAtBeginning: tail.startsAtBeginning).suffix(tailLineLimit) {
    ...
    private func readTailData(from url: URL, maxBytes: Int) throws -> (data: Data, startsAtBeginning: Bool) {
    ...
    private func tailLines(from data: Data, startsAtBeginning: Bool) -> [Data] {
        var bytes = data
