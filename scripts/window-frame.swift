...
guard CommandLine.arguments.count >= 7,
    ...
    fputs("usage: window-frame.swift <pid> <minWidth> <maxWidth> <minHeight> <maxHeight> <label>\n", stderr)
    exit(64)
}
...
let windows = info.compactMap { item -> (x: Double, y: Double, width: Double, height: Double, alpha: Double, layer: Int)? in
...
fputs("\(label) window frame lookup failed: visible windows for pid \(pid): \(windows)\n", stderr)
exit(1)
